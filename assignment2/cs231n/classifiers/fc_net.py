...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        w2 = self.params['W2']
        b2 = self.params['b2']
        o1, cache1 = affine_relu_forward(X, w1, b1)
        ...
        do1,dw2,db2 = affine_backward(dout, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i,hidden_dim in enumerate(hidden_dims):
            self.params['W'+str(i+1)] = weight_scale * np.random.randn(last_layer_dim, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        if self.use_batchnorm:
            for i in range(self.num_layers-1):
                out, cache = affine_bn_relu_forward(input, self.params['W'+str(i+1)], self.params['b'+str(i+1)],\
            ...
            for i in range(self.num_layers-1):
                ...
                dropout_caches.append(cache)
                input = out
        out, cache = affine_forward(input, self.params['W'+str(self.num_layers)], \
...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    y, cache1 = affine_forward(x, w, b)
    y_bn, cache2 = batchnorm_forward(y, gamma, beta, bn_param)
    out, cache3 = relu_forward(y_bn)
    return out, (cache1, cache2, cache3)

def affine_bn_relu_backward(dout, cache):
    cache1, cache2, cache3 = cache
    dy_bn = relu_backward(dout, cache3)
    dy, dgamma, dbeta = batchnorm_backward(dy_bn, cache2)
    dx, dw, db = affine_backward(dy, cache1)
